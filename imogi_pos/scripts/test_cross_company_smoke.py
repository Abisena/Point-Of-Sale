	...
	for doctype, field, value in (
		...
		if doctype == "IMOGI Branch":
			for name in frappe.get_all(
				"IMOGI Branch", filters={"branch_code": ["like", f"{TEST_PREFIX}%"]}, pluck="name"
			):
				try:
...
def run():
	...
	if not item:
		...
		_log("create_order", f"{order.name} company={order.company} status={order.status}")

		# Payment → POS Invoice needs open shift; verify separately when shift disabled
		settings = frappe.get_single("IMOGI POS Settings")
		if not cint(settings.enable_pos_shift):
			order.action_process_payment()
			order.reload()
			assert order.status == "Completed"
			_log("payment", f"completed total={order.grand_total}")
		else:
			_log("payment", "SKIP — shift enabled; order submitted awaiting payment")
	...
	if CLEANUP:
		...
		_log("cleanup", "orders/branch/user removed (test company retained)")
	else:
		_log("cleanup", "skipped — CLEANUP=0")
